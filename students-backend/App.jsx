...
//  Now talks to the real NestJS + PostgreSQL backend
...
export default function App() {
  ...
  // client-side filter on top of whatever the backend returned
  ...
  // ── handlers (all async now — they call the API) ─────
  async function handleAdd(data) {
    try {
      ...
      // e.g. duplicate email from the backend
          ...
          {error && (
            <div className="flex items-center gap-2.5 px-5 py-3.5 bg-red-950/40 border-b border-red-900/50">
              ...
              <p className="text-red-400 text-sm">
                Could not connect to backend: <span className="font-medium">{error}</span>
              </p>
            </div>
          )}
